...
async function CoreProofGenWithNym(PK, signature, gens, header, ph, context_id, msg_scalars,
  ...
  if(GEN_TRACE_INFO) {
    ...
    const info = {
      ...
      H: gens.generators.map(h => bytesToHex(h.toBytes(true))), // Includes Q1 at beginning
      ph: bytesToHex(ph),
...
async function NymProofInit(context_id, nym_secrets, random_scalars, api_id) {
  ...
  if(pseudonym.equals(bls.G1.Point.ZERO)) {
  ...
  if(GEN_TRACE_INFO) {
    ...
    // pid_scalar, gens, header, ph, msg_scalars, disclosed_indexes, api_id, rand_scalars
    const info = {
      info: 'from NymProofInit',
      OP: bytesToHex(OP.toBytes(true)),
      poly_eval_pseudo: numberToHex(poly_eval_pseudo, SCALAR_LENGTH),
      poly_eval_proof: numberToHex(poly_eval_proof, SCALAR_LENGTH),
      pseudonym: bytesToHex(pseudonym.toBytes(true)),
      Ut: bytesToHex(Ut.toBytes(true)),
      context_id: bytesToHex(context_id)
    };
    console.log(JSON.stringify(info, null, 2));
  }
...
export async function ProofVerifyWithNym(PK, proof, header, ph, pseudonym_bytes,
  ...
  // Deserialization:
  const pseudonym = bls.G1.Point.fromBytes(pseudonym_bytes);
...
async function NymProofVerifyInit(pseudonym, context_id, nym_secret_commitments, proof_challenge, api_id) {
  ...
  if(Uv.equals(bls.G1.Point.ZERO)) {
