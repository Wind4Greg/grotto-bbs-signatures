...
import {ProofFinalize, ProofInit, ProofVerifyInit, bytesToHex,
  calculate_random_scalars, concat, hash_to_scalar, i2osp, messages_to_scalars,
  numberToHex, octets_to_proof, octets_to_sig, prepareGenerators, serialize, verify} from './BBS.js';
...
export async function CommitWithNym(messages, prover_nyms, api_id,
...
export async function ProofGenWithNym(PK, signature, header, ph, nym_secrets, context_id, messages,
  ...
  if(disclosed_commitment_indexes.length > M + nym_secrets.length) { // allow reveal of nym secret
  ...
  const [proof, pseudonym] = await CoreProofGenWithNym(PK, signature, combinedGens, header, ph, context_id,
    combinedScalars, combined_indexes, nym_secrets.length, api_id, rand_scalars);
  // 12. return proof
...
async function NymProofVerifyInit(pseudonym, context_id, nym_secret_commitments, proof_challenge, api_id) {
  ...
  let Uv = bls.G1.ProjectivePoint.ZERO;
  for(let i = 0; i < nym_secret_commitments.length; i++) {
