...
import {calcM, CoreCommit, deserialize_and_validate_commit, FinalizeBlindSign, get_combined_idxs} from './BlindBBS.js';
import {ProofFinalize, ProofInit, ProofVerifyInit, bytesToHex, calculate_random_scalars, hash_to_scalar, messages_to_scalars, numberToHex,
  octets_to_proof, octets_to_sig, prepareGenerators, proofGen, proofVerify, serialize, verify} from './BBS.js';
...
export async function ProofGenWithNym(PK, signature, header, ph, nym_secret, context_id, messages,
  ...
  committed_message_scalars.push(bls.fields.Fr.create(nym_secret)); // counts as a committed message
  // 6.  generators = BBS.create_generators(length(message_scalars) + 1, api_id)
  const gens = await prepareGenerators(L + 1, api_id);
  // 7.  blind_generators = BBS.create_generators(length(committed_message_scalars), "BLIND_" || api_id)
...
export async function ProofVerifyWithNym(PK, proof, header, ph, pseudonym_bytes, context_id,
  ...
  // 3.  disclosed_message_scalars = messages_to_scalars(disclosed_messages, api_id)
  let msg_scalars = await messages_to_scalars(disclosed_messages, api_id);
  // 4.  disclosed_committed_message_scalars = messages_to_scalars(disclosed_committed_messages, api_id)
  const com_msg_scalars = await messages_to_scalars(disclosed_committed_messages, api_id);
  // 5.  message_scalars = disclosed_message_scalars.append(disclosed_committed_message_scalars)
  msg_scalars = msg_scalars.concat(com_msg_scalars);
  // 6.  indexes = ()
  let indexes = [];
...
/**
 * Nym secret reveal proof generation.
 *
 * @param {Uint8Array} PK - Public key as a compressed G2 point raw bytes.
 ...
 * set to 0n.
 * @param {bigint} nym_secret - The nym_secret scalar being revealed.
 * @param {string} api_id - The API id for the signature suite.
 * @param {Function} rand_scalars - A function for generating cryptographically
 *  secure random or pseudo random scalars.
 * @returns {Array} proof - An octet string or will throw an exception.
 */
export async function NymSecretProofGen(PK, signature, header, ph, messages,
  ...
  // 3. if length(disclosed_indexes) > L, return INVALID
  if(disclosed_indexes.length > L) {
    throw TypeError('NymSecretProofGen: to many disclosed indexes');
  }
  // 4. for i in disclosed_indexes, if i < 0 or i >= L, return INVALID
  for(const index of disclosed_indexes) {
    if(index < 0 || index >= L) {
      throw TypeError('NymSecretProofGen: disclosed index out of bounds');
    }
  }
  // 5. if length(disclosed_commitment_indexes) > M, return INVALID
  if(disclosed_commitment_indexes.length > M) {
    throw TypeError('NymSecretProofGen: to many disclosed commitment indexes');
  }
  // 6. for j in disclosed_commitment_indexes, if i < 0 or i >= L, return INVALID
  for(const index of disclosed_commitment_indexes) {
    if(index < 0 || index >= M) {
      throw TypeError('NymSecretProofGen: disclosed commitment index out of bounds');
    }
  }
  const gens = await prepareGenerators(L + 1, api_id);
  const blindGens = await prepareGenerators(M + 2, 'BLIND_' + api_id); // need extra generator for nym_secret
  const combinedGens = {
    P1: gens.P1,
    generators: [...gens.generators, ...blindGens.generators]
  };
  ...
  committed_message_scalars.push(nym_secret);
  ...
  // Add in index for nym_secret value
  combined_disclosed_idxs.push(L + M + 1);
 ...
 * @param {bigint} nym_secret - The scalar value nym_secret.
 ...
 */
export async function NymSecretProofVerify(PK, proof, header, ph, L,
  disclosed_messages, disclosed_committed_messages, nym_secret, disclosed_indexes,
  disclosed_committed_indexes, api_id) {
  ...
  if(proof.length < proofLenFloor) {
    if(GEN_TRACE_INFO) {
      console.log('from NymSecretProofVerify: proof is too short.');
    }
    return false;
  }
  // 3. U = floor((length(proof) - proof_len_floor) / octet_scalar_length)
  ...
  if(M === -1) { // No commitments are used!
    ...
    blindGens = await prepareGenerators(M + 2, 'BLIND_' + api_id); // one more for nym_secret
  }
  const combinedGens = {
    P1: gens.P1,
    generators: [...gens.generators, ...blindGens.generators]
  ...
  combined_idxs.push(M + L + 1); // for the nym_secret
  ...
  if(GEN_TRACE_INFO) {
    const info = {
      info: 'from NymSecretProofVerify',
