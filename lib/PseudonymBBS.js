...
export async function ProofVerifyWithNym(PK, proof, header, ph, pseudonym, context_id,
  ...
  const proofLenFloor = 3 * POINT_LENGTH + 3 * SCALAR_LENGTH; // WAS 4 * SCALAR_LENGTH
  ...
  const totalNumMessages = disclosed_indexes.length + U; // WAS + U - 1
...
async function CoreProofVerifyWithNym(PK, proof, pseudonym, context_id, gens,
  ...
  const {Abar, Bbar, D, eHat, r1Hat, r3Hat, mHatU, cp } = proof_result;
  ...
  //                  messages, ph, api_id)
  const challenge = ProofWithPseudonymChallengeCalculate(init_res, pseudonym_init_res, disclosed_indexes,
    disclosed_messages, ph, api_id);
