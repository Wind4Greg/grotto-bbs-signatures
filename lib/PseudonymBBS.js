...
export async function ProofGenWithNym(PK, signature, header, ph, nym_secret, context_id, messages,
  ...
  committed_message_scalars = committed_message_scalars.push(nym_secret);
  // 6.  generators = BBS.create_generators(length(message_scalars) + 1, api_id)
  const gens = await prepareGenerators(L + 1, api_id);
  // 7.  blind_generators = BBS.create_generators(length(committed_message_scalars) + 1, "BLIND_" || api_id)
  const blindGens = await prepareGenerators(committed_message_scalars.length + 1, 'BLIND_' + api_id);
  // 8.  indexes = ()
  ...
  const proof = await CoreProofGenWithNym(PK, signature, combinedGens, header, ph, context_id, 
    allScalars, combined_indexes, api_id);
  // 12. return proof
  return proof;
}

async function CoreProofGenWithNym(PK, signature, Pseudonym, verifier_id, generators,
  header, ph, messages, disclosed_indexes, api_id) {
  // Deserialization:
...
async function PseudonymProofInit(context_id, nym_secret, random_scalar) {
