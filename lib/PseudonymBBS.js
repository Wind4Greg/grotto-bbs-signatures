...
const TIME_PERF = true; // set to true for timing of long computations, trace should be off
let startTime; // for performance timing
let finishTime; // for performance timing

// Vector of secrets pseudonym computation options
const PSUEDO_INDEX_GROUP = 0; // create different group elements for calc
const PSEUDO_POLY_EVAL = 1; // use powers of scalar hash of context id as poly
const PSEUDO_INNER_PROD = 2; // use vector of hashes of context_id for inner prod
const PSEUDO_VECTOR_OPTION = PSEUDO_POLY_EVAL; // Set option here
...
async function NymProofInit(context_id, nym_secrets, random_scalars, api_id) {
  ...
  let pseudonym;
  let Ut;
  switch(PSEUDO_VECTOR_OPTION) {
    case PSUEDO_INDEX_GROUP:
      // 1. OPs = hash_to_curve_g1(context_id||i, api_id)
      for(let i = 0; i < nym_secrets.length; i++) {
        const OP = await hash_to_curve_g1(concat(context_id, i2osp(i, 8)), api_id);
        OPs.push(OP);
      }
      // 2. Pseudonym = sum OPs * nym_secrets
      pseudonym = bls.G1.ProjectivePoint.ZERO;
      for(let i = 0; i < nym_secrets.length; i++) {
        pseudonym = pseudonym.add(OPs[i].multiply(nym_secrets[i]));
      }
      // 3. Ut = sum OPs * random_scalars
      Ut = bls.G1.ProjectivePoint.ZERO;
      for (let i = 0; i < nym_secrets.length; i++) {
        Ut = Ut.add(OPs[i].multiply(random_scalars[i]));
      }
      break;
    case PSEUDO_POLY_EVAL:
      ...
      pseudonym = OPs[0].multiply(poly_eval_pseudo);
      Ut = OPs[0].multiply(poly_eval_proof);
      break;
    case PSEUDO_INNER_PROD:
      const OP2 = await hash_to_curve_g1(context_id, api_id);
      OPs.push(OP2);
      const dst2 = new TextEncoder().encode(api_id + 'VECT_NYM_SECRETS');
      const w = [];
      for(let i = 0; i < nym_secrets.length; i++) {
        w.push(await hash_to_scalar(concat(context_id, i2osp(i, 8)), dst2, api_id));
      }
      // there maybe more efficient algorithms for inner products?
      let inner_prod_pseudo = 0n;
      let inner_prod_proof = 0n;
      for(let i = 0; i < nym_secrets.length; i++) {
        inner_prod_pseudo = bls.fields.Fr.add(inner_prod_pseudo,
          bls.fields.Fr.mul(nym_secrets[i], w[i]));
        inner_prod_proof = bls.fields.Fr.add(inner_prod_proof,
          bls.fields.Fr.mul(random_scalars[i], w[i]));
      }
      pseudonym = OPs[0].multiply(inner_prod_pseudo);
      Ut = OPs[0].multiply(inner_prod_proof);
      break;
  }
  ...
  return [pseudonym, context_id, OPs, Ut];
}
...
async function ProofWithNymChallengeCalculate(init_res, pseudonym_init_res, i_array,
  ...
  const [pseudonym, context_id, OPs, Ut] = pseudonym_init_res;
  // ABORT if:
  ...
  if(GEN_TRACE_INFO) {
    const info = {
      ...
      OPs: OPs.map(OP => bytesToHex(OP.toRawBytes(true))),
...
async function NymProofVerifyInit(pseudonym, context_id, nym_secret_commitments, proof_challenge, api_id) {
  // Procedure:
  // 1. OPs = hash_to_curve_g1(context_id||i)
  const OPs = [];
  let Uv;
  switch(PSEUDO_VECTOR_OPTION) {
    case PSUEDO_INDEX_GROUP:
      for(let i = 0; i < nym_secret_commitments.length; i++) {
        const OP = await hash_to_curve_g1(
          concat(context_id, i2osp(i, 8)),
          api_id
        );
        OPs.push(OP);
      }
      // 2. Uv = sum {OPs * nym_secret_commitments} - Pseudonym * proof_challenge
      Uv = bls.G1.ProjectivePoint.ZERO;
      for(let i = 0; i < nym_secret_commitments.length; i++) {
        Uv = Uv.add(OPs[i].multiply(nym_secret_commitments[i]));
      }
      Uv = Uv.subtract(pseudonym.multiply(proof_challenge));
      break;
    case PSEUDO_POLY_EVAL:
      // TODO use Horner's rule. https://en.wikipedia.org/wiki/Horner%27s_method
      const OP = await hash_to_curve_g1(context_id, api_id); // What should we choose?
      OPs.push(OP);
      ...
      Uv = OPs[0].multiply(poly_eval_proof);
      Uv = Uv.subtract(pseudonym.multiply(proof_challenge));
      break;
    case PSEUDO_INNER_PROD:
      const OP2 = await hash_to_curve_g1(context_id, api_id);
      ...
      for(let i = 0; i < nym_secret_commitments.length; i++) {
        inner_prod_proof = bls.fields.Fr.add(inner_prod_proof,
          bls.fields.Fr.mul(nym_secret_commitments[i], w[i]));
      }
      Uv = OPs[0].multiply(inner_prod_proof);
      Uv = Uv.subtract(pseudonym.multiply(proof_challenge));
      break;
  }
  ...
  return [pseudonym, context_id, OPs, Uv];
}
