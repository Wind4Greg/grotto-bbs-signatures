...
function get_combined_idxs(L, disclosed_indexes, disclosed_commitment_indexes) {
  const combined_idxs = [];
...
export async function BlindProofVerify(PK, proof, header, ph, L,
  ...
  const result = proofVerify(PK, proof, header, ph, combined_scalars, combined_idxs, combinedGens, api_id);
...
export function get_disclosed_data(messages, committed_messages, disclosed_indexes,
  ...
  const indexes = [];
  for(const i of disclosed_commitment_indexes) {
    indexes.push(i);
  }
  for(const j of disclosed_commitment_indexes) {
    indexes.push(L + j + 1);
  ...
  // 5. disclosed_committed_messages = (committed_messages[j1], ..., committed_messages[jM])
