...
export async function BlindProofGen(PK, signature, header, ph, messages,
  ...
  const proof = await CoreProofGen(PK, signature, combinedGens, header, ph, message_scalars,
...
/*
[proof, add_zkp_info] = CoreProofGen(PK, signature, generators, header, ph, messages,
...
7. if disclosed_indexes and commitment_indexes are not disjoint,
  ...
  */
  ...
  // 7.      idx = commitment_indexes[i]
    ...
    const Ctilde_i = Y_0.multiply(comRandScalars(N + i)).add(Y_1.multiply(mTildeK));
...
*/
// ProofChallengeCalculate(init_res, commitment_init_res, disclosed_messages, disclosed_indexes, ph, api_id)
