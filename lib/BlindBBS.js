...
import {bytesToHex, calculate_domain, calculate_random_scalars, concat,
  hash_to_scalar, i2osp, numberToHex, octets_to_proof as bbs_octets_to_proof,
  octets_to_sig, os2ip, prepareGenerators, ProofFinalize,
  ProofInit, proofVerify, ProofVerifyInit, serialize, signature_to_octets, verify}
  from './BBS.js';
...
// TODO: Update function documentation for all messages to reflect the generalization
// of the messages  array given in the *messsages_to_scalars* function below.
 ...
 * byte arrays not strings **or** as an Array of objects of the form
 * {type: 'an elemType', value: Uint8Array}
 * elemTypes = ['Scalar', 'PlainOctets']; If the type is a 'Scalar' the
 * Uint8Array is converted to a JavaScript BigInt and no hashing is performed
 *
 * @param {string} api_id - The API id for the signature suite.
 * @returns {Array} - An array of scalars (bigint) representing the messages.
 */
export async function messages_to_scalars(messages, api_id) {
  ...
  for(let i = 0; i < messages.length; i++) {
    ...
    // Check if Uint8Array or Object with type information
    if(msg.type === undefined) { // just a Uint8Array value so hash it
      const stuff = await hash_to_scalar(msg, dst, api_id);
      scalars.push(stuff);
    } else {
      switch(msg.type) {
        case 'Scalar':
          scalars.push(os2ip(msg.value));
          break;
        case 'PlainOctets':
          const stuff = await hash_to_scalar(msg.value, dst, api_id);
          scalars.push(stuff);
          break;
      }
    }
 ...
 *
 * @param {Array} committed_messages - A vector of octet strings, messages, to be
...
export async function Commit(committed_messages, api_id,
  rand_scalars = calculate_random_scalars) {
  // 1. committed_message_scalars = BBS.messages_to_scalars(committed_messages, api_id)
  const msgScalars = await messages_to_scalars(committed_messages, api_id);
  ...
  console.log(`Create generators time = ${endTime - startTime}ms`);
 ...
 * @param {Array} committed_message_scalars - An array of message scalars for the committed messages.
 * @param {string} api_id - The API id for the signature suite.
 * @param {Function} rand_scalars - A function for generating cryptographically
 * secure random or pseudo random scalars.
 * @returns {Array}  - Comprising an octet string (Uint8Array) for the commitment with proof
 * and a the secret_prover_blind scalar in that order.
 */
export async function CoreCommit(blind_generators, committed_message_scalars, api_id, rand_scalars) {
  // Deserialization:
  ...
  const startCommit = new Date();
  let C = Q_2.multiply(secret_prover_blind);
  for(let i = 0; i < committed_message_scalars.length; i++) {
    C = C.add(J[i].multiply(committed_message_scalars[i]));
  }
  const endCommit = new Date();
  ...
  for(let i = 0; i < M; i++) {
    const mHati = bls.fields.Fr.add(m_tildes[i],
      bls.fields.Fr.mul(committed_message_scalars[i], challenge));
    mHat.push(mHati);
  }
  ...
  const endProof = new Date();
  console.log(`Commit time = ${endCommit - startCommit}ms, proof time = ${endProof - endCommit}ms`);
  ...
  if(GEN_TRACE_INFO) {
    const info = {
      ...
      Cbar: bytesToHex(Cbar.toBytes(true))
    };
...
async function CoreCommitVerify(commitment, commitment_proof, blind_generators, api_id) {
  ...
  if(GEN_TRACE_INFO) {
    const info = {
      ...
      C: bytesToHex(commitment.toBytes(true)),
      Cbar: bytesToHex(Cbar.toBytes(true))
    };
    console.log(JSON.stringify(info, null, 2));
  }
 ...
 * a octet array.
 * @param {object} blind_generators - A (blind) BBS generator object.
 * @param {string} api_id - The API id for the signature suite.
...
export async function deserialize_and_validate_commit(commitment_with_proof,
  ...
  if(blind_generators.generators.length < M + 1) {
  ...
  // 8. commitment
  const blind_generators_copy = blind_generators.generators.slice(0, M + 1);
  const res = await CoreCommitVerify(commit, commit_proof, blind_generators_copy, api_id);
  if(!res) {
...
function octets_to_commitment_with_proof(commitment_octs) {
  ...
  const C = bls.G1.Point.fromBytes(C_octets);
...
export async function BlindSign(SK, PK, commitment_with_proof, header, messages,
  ...
  const commitment = await deserialize_and_validate_commit(commitment_with_proof, blindGens, api_id);
  ...
  // 8.  (B) = res
  const B = await B_calculate(PK, gens, blindGens, commitment, message_scalars, header, api_id);
  // 9.  blind_sig = FinalizeBlindSign(SK, PK, B, api_id)
  const blind_sig = await FinalizeBlindSign(SK, B, api_id);
  // 10. if blind_sig is INVALID, return INVALID
  // 11. return blind_sig
  return blind_sig;
}

async function B_calculate(PK, generators, blind_generators, commitment,
  message_scalars, header,  api_id) {
  // Deserialization:
  // 1. L = length(messages)
  const L = message_scalars.length;
  // 2. M = length(blind_generators) - 1
  const M = blind_generators.generators.length - 1;
  // 3. if L <= 0 or M <=0, return INVALID **TO STRICT?**
  // **TODO** implement above check once we resolve issue.
  // 5. (Q_1, H_1, ..., H_L) = generators
  const [Q_1, ...H] = generators.generators;
  // 6. (Q_2, J_1, ..., J_M) = blind_generators
  const [Q_2, ...J] = blind_generators.generators;
...
export async function FinalizeBlindSign(SK, B, api_id) {
  // TODO step 1 check
  const valArray = [
  ...
  if(GEN_TRACE_INFO) {
    const info = {
      ...
      B: bytesToHex(B.toBytes(true)),
      A: bytesToHex(A.toBytes(true)),
...
export function calcM(commitment_with_proof) {
  ...
  const M = commitment_with_proof.length - POINT_LENGTH - 2 * SCALAR_LENGTH;
  if(M < 0) {
 ...
 * messages.
 * @param {Array} blind_messages - Array of byte arrays (Uint8Array) of
 ...
 *  is valid.
 */
export async function BlindVerify(PK, signature, header, messages, blind_messages,
  secret_prover_blind, api_id) {
  const L = messages.length;
  const M = blind_messages.length;
  const [allScalars, combinedGens] = await prepare_parameters(messages, blind_messages,
    L + 1, M + 1, secret_prover_blind, api_id);
 ...
 * TODO  update with: - proof, an octet string; or INVALID.
 ...
 */
/*
Note that we now have a map  rather than  index  list:
message_disclosures = {
  0: DISCLOSE,
  1: HIDE,
  2: HIDE,
...
export async function BlindProofGen(PK, signature, header, ph, messages,
  blind_messages, message_disclosures, blind_message_disclosures,
  secret_prover_blind, api_id,
  rand_scalars = calculate_random_scalars) {
  // 1. L = length(messages)
  const L = messages.length;
  ...
  // convert from message disclosure maps to lists of indices if  necessary
  let message_indexes = [];
  let blind_message_indexes = [];
  const com_message_indexes = [];
  const com_blind_message_indexes = [];
  if(Array.isArray(message_disclosures) && Array.isArray(blind_message_disclosures)) {
    message_indexes = message_disclosures;
    blind_message_indexes = blind_message_disclosures;
  } else { //convert from map to index list
    for(let i = 0; i < L; i++) { // signer messages
      if(message_disclosures[i] == 'DISCLOSE') {
        message_indexes.push(i);
      }
      if(message_disclosures[i] == 'COMMIT') {
        com_message_indexes.push(i);
      }
    }
    for(let i = 0; i < M; i++) { // prover (blind) messages
      if(blind_message_disclosures[i] == 'DISCLOSE') {
        blind_message_indexes.push(i);
      }
      if(blind_message_disclosures[i] == 'COMMIT') {
        com_blind_message_indexes.push(i);
      }
    }
  }
  if(message_indexes.length > L) {
  ...
  if(blind_message_indexes.length > M) {
  ...
  for(const index of blind_message_indexes) {
    if(index < 0 || index >= M) {
      throw TypeError('BlindProofGen: disclosed commitment index out of bounds');
    }
  }
  const [message_scalars, combinedGens] = await prepare_parameters(messages, blind_messages,
    L + 1, M + 1, secret_prover_blind, api_id);
  const combined_disclosed_idxs = get_combined_idxs(L, message_indexes,
    blind_message_indexes);
  const combined_commit_idxs = get_combined_idxs(L, com_message_indexes,
    com_blind_message_indexes);
  // CoreProofGen(PK, signature, generators, header, ph, messages, disclosed_indexes, commitment_indexes, api_id)
  const [proof, add_zkp_info] = await CoreProofGen(PK, signature, combinedGens, header, ph, message_scalars,
    combined_disclosed_idxs, combined_commit_idxs, api_id, rand_scalars);
  // **TODO** return the add_zkp_info when ready to test it
  return proof;
}

/**
 * CoreProofGen: Generates an unlinkable, selective disclosure proof based on a
 * signature and message set, and related information.
 *
 * @async
 * @param {Uint8Array} PK - Public key as a compressed G2 point raw bytes.
 * @param {Uint8Array} signature - A previously computed signature.
 * @param {object} gens - Containing an array of group G1 generators created by
 * the {@link prepareGenerators} function and the point P1.
 * @param {Uint8Array} header - Header used when signature was created.
 * @param {Uint8Array} ph - Presentation header, used during proof creation.
 ...
 * @param {Array} commitment_indexes - vector of non-negative integers in
 * ascending order. Indexes of committed messages. If not supplied, it  defaults
 * to the empty array.
 * @param {string} api_id - The API id for the signature suite.
 * @param {Function} rand_scalars - A function for generating cryptographically
 *  secure random or pseudo random scalars.
 ...
 */
async function CoreProofGen(PK, signature, gens, header, ph, messages,
  disclosed_indexes, commitment_indexes, api_id,
  rand_scalars = calculate_random_scalars
) {
  // - Y_0 and Y_1, fixed points of G1 computed as
  // (Y_0, Y_1) = BBS.create_generators(2, "COM_DIS_" || api_id)
  const comGens = await prepareGenerators(2, 'COM_DIS_' + api_id);
  const [Y_0, Y_1] = comGens.generators;
  /* TODO: indexes checks as below:
...
7. if disclosed_indexes and commitment_indexes are not disjoint,
  ...
  */
  ...
  const undisclosedIndexes = Array.from(tempSet); // these are the j_k
  ...
  // 2.  (r1, r2, e~, r1~, r3~, m~_j1, ..., m~_jU) = init_random_scalars
  const randScalars = await rand_scalars(5 + U);
  const reducedGens = {P1: gens.P1, generators: gens.generators.slice(0, L + 1)};
  const init_res = await ProofInit(PK, [A, e], reducedGens,
    randScalars, header, messages, undisclosedIndexes, api_id);
  // Calculate the commitments and initiate the correctness proof
  // 5.  (s_1, ..., s_N, s~_1, ..., s~_N) = BBS.calculate_random_scalars(2*N)
  const comRandScalars = await rand_scalars(2 * N);
  const comDis = []; // commitments for committed disclosure
  const comDisProofs = [];
  for(let i = 0; i < N; i++) {
  // 7.      idx = commitment_indexes[i]
    const idx = commitment_indexes[i];
    // 8.      C_i = Y_0 * s_i + Y_1 * messages[idx]
    const C_i = Y_0.multiply(comRandScalars[i]).add(Y_1.multiply(messages[idx]));
    comDis.push(C_i);
    // 9.      let k be the integer such that j_k == idx
    const k = undisclosedIndexes.indexOf(idx);
    const mTildeK = randScalars[5 + k];
    // 10. C~_i = Y_0 * s~_i + Y_1 * m~_k
    const sTilde_i = comRandScalars[N + i];
    const Ctilde_i = Y_0.multiply(sTilde_i).add(Y_1.multiply(mTildeK));
    comDisProofs.push(Ctilde_i);
  }
  // 11. commitment_init_res = {commitments: (C_1, ..., C_N),
  //                            commitments_proofs: (C~_1, ...,C~_N),
  //                            commitment_indexes: commitment_indexes}
  const commitment_init_res = {commitments: comDis,
    commitments_proofs: comDisProofs, commitment_indexes};
  ...
  const undisclosed_messages = messages.filter((msg, i) =>
    undisclosedIndexes.includes(i));
  // 12. challenge = ProofChallengeCalculate(init_res, commitment_init_res,
  //                 disclosed_messages, disclosed_indexes, ph, api_id)
  // 13. if challenge is INVALID, return INVALID
  const challenge = await ProofChallengeCalculate(init_res, commitment_init_res,
    disclosed_messages, disclosed_indexes, ph, api_id);
  // 14. bbs_proof = BBS.ProofFinalize(init_res, challenge, e, init_random_scalars, undisclosed_messages)
  const bbs_proof = await ProofFinalize(init_res, challenge, e, randScalars, messages,
    undisclosedIndexes, PK);
  // Finalize the commitment correctness proof
  // 15. for i in (1, 2, ..., N): s^_i =  s~_i + challenge * s_i
  const sHats = [];
  for(let i = 0; i < N; i++) {
    const s_i = comRandScalars[i];
    const sTilde_i = comRandScalars[N + i];
    const sHat_i = bls.fields.Fr.add(sTilde_i, bls.fields.Fr.mul(s_i, challenge));
    sHats.push(sHat_i);
  }
  // 16. commitments_proof = ((C_1, ..., C_N), (s^_1, ..., s^_N))
  const commitments_proof = {commitments: comDis, commitments_proof_scalars: sHats};
  // 17. proof = proof_to_octets(length(bbs_proof), bbs_proof, N, commitments_proof)
  const proof = proof_to_octets(bbs_proof.length, bbs_proof, N, commitments_proof);
  // 18. add_zkp_info = {committed_message_scalars:(messages[commitment_indexes[1]], ...,
  //                         messages[commitment_indexes[N]]), commitment_rands: (s_1, ..., s_N)}
  const add_zkp_info = {
    committed_message_scalars: messages.filter((msg, i) =>
      commitment_indexes.includes(i),
    ),
    commitment_rands: comRandScalars.slice(0, N),
  };
...
2. return oct
*/
async function proof_to_octets(bbs_proof_len, bbs_proof_octs, commitments_count,
  commitments_proof) {
  console.log(`bbs_proof_length: ${bbs_proof_len}`);
  if(commitments_count > 0) { // For now so I can use old test vectors.
    let octs = concat(i2osp(bbs_proof_len, 8), bbs_proof_octs);
    octs = concat(octs, i2osp(commitments_count, 8));
    const c_array = [];
    for(let i = 0; i < commitments_count; i++) {
      c_array.push({type: 'GPoint', value: commitments_proof.commitments[i]});
    }
    for(let i = 0; i < commitments_count; i++) {
      c_array.push({type: 'Scalar', value: commitments_proof.commitments_proof_scalars[i]});
    }
    octs = concat(octs, serialize(c_array));
    return octs;
  } else {
    return bbs_proof_octs;
  }
}

async function ProofChallengeCalculate(init_res, commitment_init_res, disclosed_messages, disclosed_indexes,
  ph, api_id
) {
  const dst = new TextEncoder().encode(api_id + 'H2S_');
  const [Abar, Bbar, D, T1, T2, domain] = init_res;
  //1. c_arr = (R, i1, msg_i1, i2, msg_i2, ..., iR, msg_iR, Abar, Bbar, D, T1, T2, domain)
  //2. c_octs = BBS.serialize(c_arr)
  const c_array = [
    {type: 'NonNegInt', value: disclosed_indexes.length}, // R
  ];
  for(let i = 0; i < disclosed_indexes.length; i++) { // i_j, msg_j
    c_array.push({type: 'NonNegInt', value: disclosed_indexes[i]});
    c_array.push({type: 'Scalar', value: disclosed_messages[i]});
  }
  ...
  let c_octs = serialize(c_array);
  // 6.  N = length(commitment_init_res.commitments)
  const N = commitment_init_res.commitments.length;
  if(N > 0) { // Do we want to skip this or process with zero N?
    const commitment_indexes = commitment_init_res.commitment_indexes;
    const commitments = commitment_init_res.commitments;
    const commitment_proofs = commitment_init_res.commitments_proofs;
    ...
    const commitment_arr = [];
    commitment_arr.push({type: 'NonNegInt', value: N});
    for(let i = 0; i < N; i++) { // i_j, msg_j
      c_array.push({type: 'NonNegInt', value: commitment_indexes[i]});
      c_array.push({type: 'GPoint', value: commitments[i]});
      c_array.push({type: 'GPoint', value: commitment_proofs[i]});
    }
    c_octs = concat(c_octs, serialize(commitment_arr));
  }

  // 5. c_octs = c_octs || I2OSP(length(ph), 8) || ph
  c_octs = concat(c_octs, i2osp(ph.length, 8));
  c_octs = concat(c_octs, ph);
 ...
 * @param {Uint8Array} header - Header used when signature was created.
 * @param {Uint8Array} ph - Presentation header, used during proof creation.
 * @param {number} L - Total number of signer messages.
...
export async function BlindProofVerify(PK, proof, header, ph, L,
  ...
  const [combined_scalars, combinedGens] = await prepare_parameters(disclosed_messages,
...
/*
result = BlindProofVerify(PK, proof, header, ph, issuer_known_messages_no, disclosed_messages, message_disclosures, api_id)
*/
/**
 *
 * @param {Uint8Array} PK - Public key as a compressed G2 point raw bytes.
 ...
 * @param {Uint8Array} header - Header used when signature was created.
 * @param {Uint8Array} ph - Presentation header, used during proof creation.
 * @param {number} issuer_known_messages_no - Total number of signer messages.
 * @param {Array} disclosed_messages - Array of byte arrays (Uint8Array) of
 * disclosed signer messages, followed by disclosed prover messages.
 * @param {Object} message_disclosures - a map between message indexes and one of the
 * DISCLOSE, HIDE or COMMIT values.
 * @param {string} api_id - The API id for the signature suite.
 */
export async function CD_BlindProofVerify(PK, proof, header, ph, issuer_known_messages_no,
  disclosed_messages, message_disclosures,  api_id) {
  // 1. bbs_proof_len = OS2IP(proof[0..7])
  const bbs_proof_len = Number(os2ip(proof.slice(0, 8))); // need regular JS number for next calcs
  console.log(`bbs_proof_len ${bbs_proof_len}`);
  // 2. undisclosed_msgs_no = (bbs_proof_len - 3 * octet_point_length - 4 * octet_scalar_length) / octet_scalar_length
  const proofLenFloor = 3 * POINT_LENGTH + 4 * SCALAR_LENGTH;
  if(proof.length < proofLenFloor) {
    if(GEN_TRACE_INFO) {
      console.log('from CD_BlindProofVerify: proof is too short.');
    }
    return false;
  }
  const undisclosed_msgs_no = Math.floor((bbs_proof_len - proofLenFloor) / SCALAR_LENGTH);
  console.log(`undisclosed_msgs_no  = ${undisclosed_msgs_no}`);
  ...
  console.log(`proof_msgs_no = ${proof_msgs_no}`);
  ...
  console.log(`total_msgs_no = ${total_msgs_no}`);
  ...
  if(Object.keys(message_disclosures).length != total_msgs_no) {
    console.log('Bad size of message_disclosures')
    return false;
  }
  // 8. if the keys of message_disclosures are not exactly the integers 0..total_msgs_no - 1, return INVALID
  // 9. if any value in message_disclosures is not one of DISCLOSE, HIDE, or COMMIT, return INVALID

  // 10. let disclosed_indexes be the integers i in 0..total_msgs_no - 1 so
  //    that message_disclosures[i] = DISCLOSE, in ascending order.
  // 11. let commitment_indexes be the integers i in 0..total_msgs_no - 1 so
  //    that message_disclosures[i] = COMMIT, in ascending order.
  const disclosed_indexes = [];
  const commitment_indexes = [];
  for(let i = 0; i < total_msgs_no; i++) {
    if(message_disclosures[i] == 'DISCLOSE') {
      disclosed_indexes.push(i);
    }
    if(message_disclosures[i] == 'COMMIT') {
      commitment_indexes.push(i);
    }
  }
  console.log('disclosed_indexes:');
  console.log(disclosed_indexes);
  console.log('commitment_indexes:');
  console.log(commitment_indexes);
  // 12. if length(disclosed_indexes) != length(disclosed_messages), return INVALID
  if(disclosed_indexes.length != disclosed_messages.length) {
    console.log('Issue with length of disclosed_indexes versus disclosed_messages');
    return false;
  }
  // 13. let proof_index(i) be i if i < issuer_known_messages_no, and i + 1 otherwise.
  ...
  const proof_disclosed_indexes = disclosed_indexes.map(value => {
    if(value < issuer_known_messages_no) {
      return value;
    } else {
      return value + 1;
    }
  });
  const proof_commitment_indexes = commitment_indexes.map(value => {
    if(value < issuer_known_messages_no) {
      return value;
    } else {
      return value + 1;
  ...
  console.log(proof_disclosed_indexes);
  //   Procedure:
  // 1. generators = BBS.create_generators(issuer_known_messages_no + 1, api_id)
  // 2. blind_generators = BBS.create_generators(total_msgs_no - issuer_known_messages_no + 1, "BLIND_" || api_id)
  // 3. message_scalars = BBS.messages_to_scalars(disclosed_messages, api_id)
  const gens = await prepareGenerators(issuer_known_messages_no + 1, api_id);
  const blindGens = await prepareGenerators(total_msgs_no - issuer_known_messages_no + 1, 'BLIND_' + api_id);
  ...
  // **TODO**
  // 4. result = CoreProofVerify(PK, proof, generators.append(blind_generators), header, ph, message_scalars,
  // proof_disclosed_indexes, proof_commitment_indexes, api_id)
  console.log('Calling CoreProofVerify');
  const result = await CoreProofVerify(PK, proof, combinedGens, header, ph,
    message_scalars, proof_disclosed_indexes, proof_commitment_indexes, api_id);
  return result;
}
...
async function CoreProofVerify(PK, proof, generators, header, ph,
  ...
  const W = bls.G2.Point.fromBytes(PK);
  ...
  // console.log('bbs_proof_res:');
  // console.log(bbs_proof_res);
  // console.log('commitments_proof_res:');
  // console.log(commitments_proof_res);
  // Note my bbs_proof_res = {Abar,  Bbar, D, eHat,  r1Hat, r3Hat, mHatU, c}
  // Note my commitments_proof_res = {commitments, s_vect}
  // 6.  (Abar, Bbar, D, e^, r1^, r3^, hats, cp) = bbs_proof_res
  const {Abar, Bbar, D, eHat, r1Hat,  r3Hat, mHatU:hats, c:cp} = bbs_proof_res;
  console.log(hats);
  // 7.  (commitments, commitments_proof) = commitments_proof_res
  const commitments = commitments_proof_res.commitments;
  const commitments_proof = commitments_proof_res.s_vect;
  ...
  // 14. L = R + U
  const L = R + U; // all messages including secret prover blind
  // Recall that undisclosed includes secret prover blind which in the total list comes after issuer messages.
  console.log(`N = ${N}, U = ${U}, R = ${R}, Recovered L  = ${L}`);
  ...
  //   Procedure:
  // 1.  init_res = BBS.ProofVerifyInit(PK, bbs_proof_res, generators, header, disclosed_messages, disclosed_indexes, api_id)
  // async function ProofVerifyInit(PK, proof_result, gens, header,  disclosed_messages, disclosed_indexes, api_id)
  ...
  for(let i = 0; i < N; i++) {
    ...
    const k = undisclosed_indexes.indexOf(idx);
    console.log(`idx =  ${idx}, k = ${k}`);
    // 6.      C^_i = Y_0 * s^_i + Y_1 * m^_k - C_i * cp
    const sHat_i = commitments_proof[i];
    const mHat_k = hats[k];
    let CHat_i = Y_0.multiply(sHat_i).add(Y_1.multiply(mHat_k));
    CHat_i = CHat_i.subtract(commitments[i].multiply(cp));
    CHats.push(CHat_i);
  }
  // console.log('CHats:');
  // console.log(CHats);
  // 7.  commitment_init_res = {commitments: (C_1, ..., C_N), commitments_proofs: (C^_1, ...,C^_N), commitment_indexes: commitment_indexes}
  const commitment_init_res = {commitments, commitments_proofs:CHats, commitment_indexes};
  // 8.  challenge = ProofChallengeCalculate(init_res, commitment_init_res, disclosed_messages, disclosed_indexes, ph, api_id)
  const challenge = await ProofChallengeCalculate(init_res, commitment_init_res, disclosed_messages, disclosed_indexes,
    ph, api_id);
  console.log(`challenge = ${challenge}, cp = ${cp}`);
  ...
  // 6. return VALID
  // Compute item in G2
  const negP2 = bls.G2.Point.BASE.negate();
  ...
  const verified = bls.fields.Fp12.eql(result, bls.fields.Fp12.ONE);
...
*/
function octets_to_proof(proof_octets) {
  // console.log('proof_octets in  hex:');
  // console.log(bytesToHex(proof_octets));
  const INT_OCTET_LENGTH = 8;
  //   Procedure:
  // 1.  sidx = 0
  let sidx = 0;
  // 2.  eidx = int_octet_length - 1
  let eidx = INT_OCTET_LENGTH; // Using JavaScript slice which doesnt include end
  // 3.  if length(proof_octets) < eidx, return INVALID
  if(proof_octets.length < eidx) {
    console.log('Something is wrong with proof_octets');
    return null;
  }
  // 4.  bbs_proof_len = OS2IP(proof_octets[sidx, ..., eidx])
  const bbs_proof_len = Number(os2ip(proof_octets.slice(sidx, eidx)));
  // 5.  sidx = eidx + 1
  sidx = eidx;
  ...
  if(proof_octets.length < eidx) {
    console.log("problem with proof_octets and bbs_proof_len");
    return null;
  }
  // 8.  bbs_proof_octs = proof_octets[sidx, ..., eidx]
  const bbs_proof_octs = proof_octets.slice(sidx, eidx);
  // console.log(`bbs_proof starting index: ${sidx}, ending:  ${eidx} `);
  // console.log(`length bbs_proof_octs:  ${bbs_proof_octs.length}`);
  // console.log('bbs_proof_octs in hex:');
  // console.log(bytesToHex(bbs_proof_octs));

  // 9.  bbs_proof = BBS.octets_to_proof(bbs_proof_octs)
  const bbs_proof = bbs_octets_to_proof(bbs_proof_octs);
  // console.log('bbs_proof:');
  // console.log(bbs_proof);
  ...
  if(proof_octets.length < eidx) {
    console.log('Issue with proof_octets length and getting  N value');
    return null;
  }
  // 14. N = OS2IP(proof_octets[sidx, ..., eidx]) // commitments count
  const N = Number(os2ip(proof_octets.slice(sidx, eidx)));
  console.log(`recovered N = ${N}`);

  // 15. len_floor = eidx + N * (octet_point_length + octet_scalar_length)
  const len_floor = eidx + N * (POINT_LENGTH + SCALAR_LENGTH);
  console.log(`len_floor = ${len_floor}, proof_length  = ${proof_octets.length}`);
  // 16. if length(proof_octets) < len_floor, return INVALID
  if(proof_octets.length < len_floor) {
    console.log('Issue with proof_octs and commitments proof');
    return null;
  }
  // 17. for i in (1..N)
  const Cs = [];
  for(let i = 0; i < N; i++) {
    ...
    eidx = sidx + POINT_LENGTH;
    // 20.     C_i = BBS.octets_to_point_E1(proof_octets[sidx, ..., eidx])
    const C_i = bls.G1.Point.fromBytes(proof_octets.slice(sidx, eidx));
  ...
  for(let i = 0; i < N; i++) {
    ...
    sidx = eidx;
    // 25.     eidx = sidx + ;
    eidx = sidx + SCALAR_LENGTH;
    // 26.     s_i = OS2IP(proof_octets[sidx, ..., eidx])
    const s_i = os2ip(proof_octets.slice(sidx, eidx));
  ...
  const commitments_proof = {commitments: Cs, s_vect};
...
/**
 *
 * @param {Array} messages - A vector of octet strings. If not supplied, it
 * defaults to the empty array.
 * @param {Array} committed_messages - A vector of octet strings. If not
 * supplied, it defaults to the empty array.
 * @param {number} generators_number - The number of "regular" generators to prepare.
 * @param {number} blind_generators_number - The number of "blind" generators to prepare.
 * @param {bigint} secret_prover_blind - A scalar value or NONE (null).
 * @param {string} api_id - The API id for the signature suite.
 * @returns {Array} - An array, [message_scalars, generators], containing message_scalars
 * of scalar values and a vector generators of points from the G1 subgroup.
 */
export async function prepare_parameters(messages, committed_messages,
  generators_number, blind_generators_number, secret_prover_blind, api_id) {
  ...
  const gens = await prepareGenerators(generators_number, api_id);
  // 6. blind_generators = BBS.create_generators(blind_generators_number, "BLIND_" || api_id)
  const blindGens = await prepareGenerators(blind_generators_number, 'BLIND_' + api_id);
  ...
  return [allScalars, combinedGens];
}
