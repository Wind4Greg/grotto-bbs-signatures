...
import {bytesToHex, calculate_random_scalars, concat, hash_to_scalar, messages_to_scalars,
  os2ip, prepareGenerators, serialize, signature_to_octets}
  from './BBS.js';
...
export async function BlindSign(SK, PK, commitment_with_proof, header, messages,
  ...
  console.log(`Blind sign: L = ${L}, M = ${M}`);
  // Procedure:
...
/*
CoreBlindSign(SK, PK, generators, commitment_with_proof, header, messages, signer_blind, api_id)
...
async function CoreBlindSign(SK, PK, commitment_with_proof, gens, header,
  ...
  const commit_res = await deserialize_and_validate_commit(commitment_with_proof, gens, api_id);
  let [commit, M] = commit_res;
  console.log(`In core blind sign: L = ${L}, M = ${M}`);
  // 5. Q_1 = generators[0]
  ...
  console.log(`In core blind H.length = ${H.length}`);
  // Procedure:
  ...
  console.log('H:');
  console.log(H);
  const domain = await calculate_domain(PK, gens.length, Q_1, gens, header, api_id);
  ...
  // console.log("Checking e_serial_octs:");
  // console.log(e_serial_octs);
  // 3. e = BBS.hash_to_scalar(e_octs || commitment_with_proof, signature_dst)
  const sig_dst = new TextEncoder().encode('H2S_' + api_id);
  const e = await hash_to_scalar(concat(e_serial_octs, commitment_with_proof), sig_dst, api_id);
  // // if a commitment is not supplied, Q_2 = Identity_G1, meaning that
  // // signer_blind will be ignored.
  // 4. commit = commit + Q_2 * signer_blind
  console.log('In core blind sign, Q_2:');
  console.log(Q_2);
...
/**
 * Helper function to give the number of committed messages in a commitment with
 * proof.
 *
...
async function calculate_domain(PK, L, Q_1, H_Points, header, api_id) {
  ...
  const dst = new TextEncoder().encode(api_id + 'H2S_');
  console.log(`calc_dom dom_for_hash: ${bytesToHex(dom_for_hash)}`);
