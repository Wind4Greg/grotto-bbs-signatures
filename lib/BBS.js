...
export async function proofGen(PK, signature, header, ph, messages,
  ...
  // r2^ = r2 + r4 * c (mod r)
  const r2Hat = bls.fields.Fr.add(r2, bls.fields.Fr.mul(r4, c));
  // r3^ = r3 + e * r4 * c (mod r)
  const r3Hat = bls.fields.Fr.add(r3, bls.fields.Fr.mul(e,
  ...
  // proof = (Abar, Bbar, c, r2^, r3^, (m^_j1, ..., m^_jU))
  // return proof_to_octets(proof)
  return proof_to_octets(Abar, Bbar, c, r2Hat, r3Hat, mHatU);
}
...
export async function proofVerify(PK, proof, header, ph, disclosed_messages,
  ...
  const {Abar, Bbar, c, r2Hat, r3Hat, mHatU} = proof_result;
  const R = disclosed_indexes.length;
  ...
  let C = Bbar.multiply(r2Hat).add(Abar.multiply(r3Hat));
  for(let j = 0; j < U; j++) {
...
function octets_to_proof(octets) {
  // recover (Abar, Bbar, c, r2Hat, r3Hat, mHatU) from octets
  ...
  return {Abar, Bbar, c, r2Hat, r3Hat, mHatU};
}

// proof_to_octets(Abar, Bbar, c, r2Hat, r3Hat, mHatU);
function proof_to_octets(Abar, Bbar, c, r2Hat, r3Hat, mHatU) {
