...
export async function ProofInit(PK, signature, gens, randScalars, header,
  ...
  if(GEN_TRACE_INFO) {
    ...
    const info = {
      ...
      domain: numberToHex(domain, SCALAR_LENGTH),
      undisclosed_indexes,
      Hundisclosed: undisclosed_indexes.map(u => bytesToHex(H[u].toRawBytes(true))),
      msg_scalars: messages.map(m => numberToHex(m, SCALAR_LENGTH))
    };
    console.log(JSON.stringify(info, null, 2));
  }
...
export async function ProofVerifyInit(PK, proof_result, gens, header,
  ...
  if(GEN_TRACE_INFO) {
    const info = {
      ...
      Hundisclosed: undisclosed.map(u => bytesToHex(H[u].toRawBytes(true))),
...
export async function prepareGenerators(count, api_id) {
  ...
  if(GEN_TRACE_INFO) {
    ...
    const info = {
      ...
      count,
      api_id,
      P1: bytesToHex(gen.P1.toRawBytes(true)),
      Q1: bytesToHex(Q1.toRawBytes(true)),
      MsgGenerators: H.map(h => bytesToHex(h.toRawBytes(true)))
    };
    console.log(JSON.stringify(info, null, 2));
  }
  return gen;
