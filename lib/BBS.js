...
export async function proofGen(PK, signature, header, ph, messages,
  ...
  const init_res = await ProofInit(PK, [A, e], gens,
    randScalars, header, messages, undisclosed, api_id);
  ...
  return ProofFinalize(init_res, c, e, randScalars, messages, undisclosed);
}
...
export function ProofFinalize(init_res, challenge, e, randScalars, messages,
  undisclosed) {
  if(GEN_TRACE_INFO) {
    const info = {challenge: numberToHex(challenge, SCALAR_LENGTH)};
    console.log(JSON.stringify(info, null, 2));
  }
  // ProofFinalize:
  const [r1, r2, eTilde, r1Tilde, r3Tilde, ...mTildeU] = randScalars;
  ...
  const mHatU = [];
  for(let j = 0; j < undisclosed.length; j++) {
    const mHatj = bls.fields.Fr.add(mTildeU[j],
      bls.fields.Fr.mul(messages[undisclosed[j]], challenge));
    mHatU.push(mHatj);
  }
