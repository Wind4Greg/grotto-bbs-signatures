...
export const API_ID_PSEUDONYM_BBS_SHA = CIPHERSUITE_ID_SHA +
  'H2G_HM2S_PSEUDONYM_';
export const API_ID_PSEUDONYM_BBS_SHAKE = CIPHERSUITE_ID_SHAKE +
  'H2G_HM2S_PSEUDONYM_';
...
export function publicFromPrivate(privateBytes) {
  // console.log("dumping G2 base:");
  // console.log(bls.G2.Point.BASE.toHex());
  const pointPk = bls.G2.Point.BASE.multiply(os2ip(privateBytes));
  return pointPk.toBytes(true);
 ...
 * @param {object} gen - Contains array of group G1 generators created by the
 *  {@link prepareGenerators} function and the point P1.
 * @param {string} api_id - The API id for the signature suite.
 */
export async function sign(SK, PK, header, messages, gen, api_id) {
  if(typeof SK !== 'bigint') { // if byte array convert to BigInt
    SK = os2ip(SK);
  }
  // check that we have enough generators for the messages
  if(messages.length > gen.generators.length + 1) {
    throw new TypeError('Sign: not enough generators! string');
  }
  const L = messages.length;
  const dst = new TextEncoder().encode(api_id + 'H2S_');
  const [Q1, ...H] = gen.generators.slice(0, L + 1);
  const domain = await calculate_domain(PK, Q1, H, header, api_id);
  ...
  let B = gen.P1;
  B = B.add(Q1.multiply(domain));
  for(let i = 0; i < messages.length; i++) {
    B = B.add(H[i].multiply(messages[i]));
  }
  // A = B * (1 / (SK + e))
 ...
 * @param {Array} messages - Array of scalars (bigint) derived from actual
 *  messages. Computed by {@link messages_to_scalars}.
 * @param {object} gens - Contains an array of group G1 generators created by
 * the {@link prepareGenerators} function and the point P1.
 * @param {string} api_id - The API id for the signature suite.
 * @returns {boolean} - True or False depending on whether the signature
 *  is valid.
 */
export async function verify(PK, signature, header, messages, gens,
  api_id) {
  ...
  const L = messages.length;
  const [Q1, ...H] = gens.generators.slice(0, L + 1);
  const domain = await calculate_domain(PK, Q1, H, header, api_id);
  // B = P1 + Q_1 * domain + H_1 * msg_1 + ... + H_L * msg_L
  let B = gens.P1;
  B = B.add(Q1.multiply(domain));
  for(let i = 0; i < messages.length; i++) {
    if(messages[i] !== 0n) { // Noble curves won't multiply by zero.
      B = B.add(H[i].multiply(messages[i]));
    }
  }
  ...
  // Compute items in G2
  const temp1G2 = W.add(bls.G2.Point.BASE.multiply(e));
  const temp2G2 = bls.G2.Point.BASE.negate();
  ...
  if(GEN_TRACE_INFO) {
    const info = {
      ...
      e: numberToHex(e, SCALAR_LENGTH),
      msg_scalars: messages.map(m => numberToHex(m, SCALAR_LENGTH)),
      H: H.map(Hi => bytesToHex(Hi.toBytes(true))),
      Q1: bytesToHex(Q1.toBytes(true))
    };
    console.log(JSON.stringify(info, null, 2));
  }
...
export async function proofGen(PK, signature, header, ph, messages,
  ...
  const proof = await ProofFinalize(init_res, c, e, randScalars, messages,
    undisclosedIndexes, PK);
  return proof;
}

export async function ProofInit(PK, signature, gens, randScalars, header,
  ...
  // console.log('all random scalars:');
  // console.log(randScalars.map(rs => rs.toString(16)));
  const [Q1, ...H] = gens.generators;
  const domain = await calculate_domain(PK, Q1, H, header, api_id); // DIFFERENT FROM  ProofVerifyInit
  ...
  for(let j = 0; j < U; j++) {
    T2 = T2.add(H[undisclosed_indexes[j]].multiply(mTildeU[j]));
  }
  if(GEN_TRACE_INFO) {
    // do paring check here too...
    const W = bls.G2.Point.fromBytes(PK);
    ...
    let result = bls.fields.Fp12.mul(ptGT1, ptGT2);
    result = bls.fields.Fp12.finalExponentiate(result); // See noble BLS12-381
    const valid = bls.fields.Fp12.eql(result, bls.fields.Fp12.ONE);
    const info = {
      info: 'from ProofInit',
      check: `pairing based check: ${valid}`,
      U, L,
      randomScalars: {
        r1: numberToHex(r1, SCALAR_LENGTH),
        r2: numberToHex(r2, SCALAR_LENGTH),
        e_Tilde: numberToHex(eTilde, SCALAR_LENGTH),
        r1_Tilde: numberToHex(r1Tilde, SCALAR_LENGTH),
        r3_Tilde: numberToHex(r3Tilde, SCALAR_LENGTH),
        m_tilde_scalars: mTildeU.map(mT => numberToHex(mT, SCALAR_LENGTH))
      },
      A: bytesToHex(A.toBytes(true)),
      Abar: bytesToHex(Abar.toBytes(true)),
      B: bytesToHex(B.toBytes(true)),
      Bbar: bytesToHex(Bbar.toBytes(true)),
      D: bytesToHex(D.toBytes(true)),
      T1: bytesToHex(T1.toBytes(true)),
      T2: bytesToHex(T2.toBytes(true)),
      domain: numberToHex(domain, SCALAR_LENGTH),
      undisclosed_indexes,
      Hundisclosed: undisclosed_indexes.map(u => bytesToHex(H[u].toBytes(true))),
      msg_scalars: messages.map(m => numberToHex(m, SCALAR_LENGTH))
    };
    console.log(JSON.stringify(info, null, 2));
  }
...
// Note PK is only used in a consistency check if GEN_TRACE_INFO is true
export function ProofFinalize(init_res, challenge, e, randScalars, messages,
  undisclosedIndexes, PK) {
  // ProofFinalize:
  const [r1, r2, eTilde, r1Tilde, r3Tilde, ...mTildeU] = randScalars;
  // 1. r3 = r2^-1 (mod r)
  ...
  const eHat = bls.fields.Fr.add(eTilde, bls.fields.Fr.mul(e, challenge));
  // 3. r1^ = r1~ - r1 * challenge
  const r1Hat = bls.fields.Fr.sub(r1Tilde, bls.fields.Fr.mul(r1, challenge));
  // 4. r3^ = r3~ - r3 * challenge
  const r3Hat = bls.fields.Fr.sub(r3Tilde, bls.fields.Fr.mul(r3, challenge));
  // 5. for j in (1, ..., U): m^_j = m~_j + undisclosed_j * challenge (mod r)
  const mHatU = [];
  for(let j = 0; j < undisclosedIndexes.length; j++) {
    const mHatj = bls.fields.Fr.add(mTildeU[j],
      bls.fields.Fr.mul(messages[undisclosedIndexes[j]], challenge));
    mHatU.push(mHatj);
  }
  const [Abar, Bbar, D] = init_res;
  if(GEN_TRACE_INFO) {
    // Check if values are consistent, i.e., via the pairing check from verify
    const W = bls.G2.Point.fromBytes(PK);
    const negP2 = bls.G2.Point.BASE.negate();
    const ptGT1 = bls.pairing(Abar, W);
    const ptGT2 = bls.pairing(Bbar, negP2);
    ...
    const valid = bls.fields.Fp12.eql(result, bls.fields.Fp12.ONE);
    const info = {
      info: 'from ProofFinalize',
      check: `pairing based check: ${valid}`,
      challenge: numberToHex(challenge, SCALAR_LENGTH),
      mHatU: mHatU.map(mHat => numberToHex(mHat, SCALAR_LENGTH)),
      undisclosed_msg_scalars: undisclosedIndexes.map(i =>
        numberToHex(messages[i], SCALAR_LENGTH))
    };
    console.log(JSON.stringify(info, null, 2));
  }
  return proof_to_octets(Abar, Bbar, D, eHat, r1Hat, r3Hat, mHatU, challenge);
}
...
export async function proofVerify(PK, proof, header, ph, disclosed_messages,
  ...
  // 1. proof_len_floor = 2 * octet_point_length + 4 * octet_scalar_length // error  2 -> 3
  const proof_len_floor = 3 * POINT_LENGTH + 4 * SCALAR_LENGTH;
  // 2. if length(proof) < proof_len_floor, return INVALID
  if(proof.length < proof_len_floor) {
    return false;
  }
  // 3. U = floor((length(proof) - proof_len_floor) / octet_scalar_length)
  const U = Math.floor((proof.length - proof_len_floor) / SCALAR_LENGTH);
  // 4. R = length(disclosed_indexes)
  const R = disclosed_indexes.length;
  const L = U + R;
  // Check and prepare generators
  if(gens.generators.length < L + 1) {
    throw new TypeError(`Sign: not enough generators! L =  ${L}, U = ${U}, R = ${R} Length  =  ${gens.generators.length}`);
  }
  // Setup reduced sized generators
  const reducedGens = {P1: gens.P1, generators: gens.generators.slice(0, L + 1)};
  ...
  let {Abar, Bbar, D, c} = proof_result;
  ...
  let T1; let T2; let domain;
  [Abar, Bbar, D, T1, T2, domain] = await ProofVerifyInit(PK,
    proof_result, reducedGens, header, disclosed_messages, disclosed_indexes, api_id);
...
export async function ProofVerifyInit(PK, proof_result, gens, header,
  ...
  let Bv = gens.P1;
  Bv = Bv.add(Q1.multiply(domain));
  for(let i = 0; i < R; i++) {
    Bv = Bv.add(
      H[disclosed_indexes[i]].multiply(disclosed_messages[i])
    );
  }
  ...
  for(let j = 0; j < U; j++) {
    T2 = T2.add(H[undisclosed[j]].multiply(mHatU[j]));
  }
  if(GEN_TRACE_INFO) {
    const info = {
      ...
      T2: bytesToHex(T2.toBytes(true)),
      domain: numberToHex(domain, SCALAR_LENGTH),
      challengeProof: numberToHex(c, SCALAR_LENGTH),
      undisclosed_indexes: undisclosed,
      Hundisclosed: undisclosed.map(u => bytesToHex(H[u].toBytes(true))),
      disclose_messages: disclosed_messages.map(m =>
        numberToHex(m, SCALAR_LENGTH))
    };
    console.log(JSON.stringify(info, null, 2));
  }
  return [Abar, Bbar, D, T1, T2, domain];
...
export function octets_to_proof(octets) {
  ...
  const Abar = bls.G1.Point.fromBytes(Abar_oct);
  index += POINT_LENGTH;
  const Bbar_oct = octets.slice(index, index + POINT_LENGTH);
  const Bbar = bls.G1.Point.fromBytes(Bbar_oct);
  index += POINT_LENGTH;
  const D_oct = octets.slice(index, index + POINT_LENGTH);
  const D = bls.G1.Point.fromBytes(D_oct);
  index += POINT_LENGTH;
...
function proof_to_octets(Abar, Bbar, D, eHat, r1Hat, r3Hat, mHatU, c) {
  let octets = Abar.toBytes(true);
  octets = concat(octets, Bbar.toBytes(true));
  octets = concat(octets, D.toBytes(true));
...
export function signature_to_octets(A, e) {
  let octets = A.toBytes(true);
  octets = concat(octets, numberToBytesBE(e, SCALAR_LENGTH));
  return octets;
}

export async function calculate_domain(PK, Q_1, H_Points, header, api_id) {
  ...
  if(GEN_TRACE_INFO) { //PK, L, Q_1, H_Points, header, api_id
    const info = {
      ...
      PK: bytesToHex(PK),
      L,
      Q_1: bytesToHex(Q_1.toBytes(true)),
      H: H_Points.map(Hi => bytesToHex(Hi.toBytes(true))),
...
export async function calculate_challenge(Abar, Bbar, D, T1, T2, i_array, msg_array,
  domain, ph, api_id
) {
  const dst = new TextEncoder().encode(api_id + 'H2S_');
  const c_array = [
  ...
  for(const element of elem_array) {
    switch(element.type) {
      ...
      case 'NonNegInt':
        ...
        break;
      case 'GPoint':
        octets = concat(octets, element.value.toBytes(true));
        break;
      case 'Scalar':
 ...
 *
 * @async
 * @param {number} count - An integer that indicates the number of generators to
 * be created. This number must be large than the total number of messages in a
 * signature or proof.
 * @param {string} api_id - The API id for the signature suite.
 * @returns {object} - Containing an array of group generators used by the
 * signature/proof suite, and the point P1.
 */
export async function prepareGenerators(count, api_id) {
  // Compute P1, generators[0, count-1]
  ...
  if(GEN_TRACE_INFO) {
    ...
    const info = {
      info: 'from prepareGenerators',
      count,
      api_id,
      P1: bytesToHex(gen.P1.toBytes(true)),
      Q1: bytesToHex(Q1.toBytes(true)),
      MsgGenerators: H.map(h => bytesToHex(h.toBytes(true)))
    };
    console.log(JSON.stringify(info, null, 2));
  }
  return gen;
...
export async function seeded_random_scalars(seed, dst, count) {
  // let ciphersuite_id = CIPHERSUITE_ID;
  // if(hashType === 'SHAKE-256') {
  //   ciphersuite_id = CIPHERSUITE_ID_SHAKE;
  // }
  const te = new TextEncoder();
  const MOCK_DST = te.encode(dst);
  // 1. out_len = expand_len * count
  const out_len = EXPAND_LEN * count;
  // 2. v = expand_message(SEED, dst, out_len)
  let v;
  if(dst.includes('SHA-256')) {
