...
export async function sign(SK, PK, header, messages, generators, hashType = "SHA-256") {
    ...
    let e_s_expand;
    if (hashType === "SHA-256") {
        e_s_expand  = await bls.utils.expandMessageXMD(e_s_octs, expand_dst, 2*EXPAND_LEN);
    } else {
        e_s_expand  = expandMessageXOF(e_s_octs, expand_dst, 2*EXPAND_LEN);
    }
    // 7.  if e_s_expand is INVALID, return INVALID
    // 8.  e = hash_to_scalar(e_s_expand[0..(expand_len - 1)])
    let [e] = await hash_to_scalar(e_s_expand.slice(0, EXPAND_LEN), 1, dst, hashType);
    // 9.  s = hash_to_scalar(e_s_expand[expand_len..(expand_len * 2 - 1)])
    let [s] = await hash_to_scalar(e_s_expand.slice(EXPAND_LEN, 2*EXPAND_LEN), 1, dst, hashType);
...
export async function proofVerify(PK, proof, L, header, ph, disclosed_messages, disclosed_indexes, generators, hashType="SHA-256") {
    let ciphersuite_id = CIPHERSUITE_ID;
    if (hashType === "SHAKE-256") {
        ciphersuite_id = CIPHERSUITE_ID_SHAKE;
    }
...
async function calculate_challenge(Aprime, Abar, D, C1, C2, i_array, msg_array, domain, ph, hashType="SHA-256") {
    ...
    let [c] = await hash_to_scalar(c_for_hash, 1, dst, hashType);
    return c;
}
