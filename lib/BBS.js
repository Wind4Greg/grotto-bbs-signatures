...
export async function proofGen(PK, signature, header, ph, messages, disclosed_indexes, generators, rand_scalars = calculate_random_scalars) {
    ...
    let domain = await calculate_domain(PK, L, generators.Q1, generators.Q2, generators.H, header);
    ...
    let c = await calculate_challenge(Aprime, Abar, D, C1, C2, disclosed_indexes, disclosed_messages, domain, ph)
    // 22. e^ = c * e + e~ mod r
...
export async function proofVerify(PK, proof, L, header, ph, disclosed_messages, disclosed_indexes, generators) {
    ...
    let cv = await calculate_challenge(Aprime, Abar, D, C1, C2, disclosed_indexes, disclosed_messages, domain, ph)
...
async function calculate_challenge(Aprime, Abar, D, C1, C2, i_array, msg_array, domain, ph) {
    let dst = new TextEncoder().encode(CIPHERSUITE_ID + "H2S_");
...
export async function hash_to_scalar(msg_octets, count, dst, hashType = "SHA-256") {
