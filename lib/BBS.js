...
export async function proofGen(PK, signature, header, ph, messages,
  ...
  const disclosed_messages = messages.filter((msg, i) =>
    ...
    // For proof verification debug only (remove Bv stuff later):
  // 3. Bv = P1 + Q_1 * domain + H_i1 * msg_i1 + ... + H_iR * msg_iR
  let Bv = generators.P1;
  Bv = Bv.add(generators.Q1.multiply(domain));
  for(let i = 0; i < R; i++) {
    Bv = Bv.add(
      generators.H[disclosed_indexes[i]].multiply(
        messages[disclosed_indexes[i]])
    );
  }
  console.log(`Bv: ${bytesToHex(Bv.toRawBytes(true))}`);
  console.log('Disclosed Indexes:');
  console.log(disclosed_indexes);
  const disMsgHex = disclosed_messages.map(scalar => scalar.toString(16));
  console.log('Disclosed msgs:');
  console.log(disMsgHex);
  //
  ...
  const mHatUhex = mHatU.map(scalar => scalar.toString(16));
  // console.log('mHatU in hex:');
  // console.log(mHatUhex);
...
/*
calculate_challenge(Abar, Bbar, D, T1, T2, i_array, msg_array,
...
*/
  ...
  console.log(`Domain: ${domain.toString(16)}`);
  console.log(`Challenge  (c): ${c.toString(16)}`);
