...
/**
 *
 * @param {bigint} SK - A scalar for the secret key.
...
export async function sign(SK, PK, header, messages, generators,
  ...
  let e_s_expand;
  if(hashType === 'SHA-256') {
    e_s_expand = await expand_message_xmd(
      e_s_octs,
      expand_dst,
      2 * SCALAR_LENGTH,
      sha256
  ...
  const denom = bls.Fr.add(bls.Fr.create(SK), bls.Fr.create(e));
  const num = bls.Fr.inv(denom);
...
export async function proofGen(PK, signature, header, ph, messages,
  ...
  const eHat = bls.Fr.add(bls.Fr.mul(c, e), eTilde);
  // 23. r2^ = c * r2 + r2~ mod r
  const r2Hat = bls.Fr.add(bls.Fr.mul(c, r2), r2Tilde);
  // 24. r3^ = c * r3 + r3~ mod r
  const r3Hat = bls.Fr.add(bls.Fr.mul(c, r3), r3Tilde);
  // 25. s^ = c * s' + s~ mod r
  const sHat = bls.Fr.add(bls.Fr.mul(c, sPrime), sTilde);
  // 26. for j in (j1, ..., jU): m^_j = c * msg_j + m~_j mod r
  const mHatU = [];
  for(let j = 0; j < U; j++) {
    const mHatj = bls.Fr.add(bls.Fr.mul(c, messages[undisclosed[j]]),
      mTildeU[j]);
    mHatU.push(mHatj);
  }
...
async function calculate_challenge(
  Aprime,
  Abar,
  D,
  C1,
...
export function calculate_random_scalars(count) {
  ...
  const scalars = [];
  for(let i = 0; i < count; i++) {
    const r_i = os2ip(randomBytes(EXPAND_LEN)) % bls.CURVE.r;
    scalars.push(r_i);
  }
  return scalars;
}
...
export async function seeded_random_scalars(seed, hashType, count) {
  ...
  for(let i = 0; i < count; i++) {
    ...
    const scalar_i = os2ip(tv) % bls.CURVE.r;
    scalars[i] = scalar_i;
  }
