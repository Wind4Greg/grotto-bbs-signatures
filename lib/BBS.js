...
export async function sign(SK, PK, header, messages, generators, hashType = "SHA-256") {
    ...
    let e_s_expand;
    const TEMP_EX_LEN = 32;
    if (hashType === "SHA-256") {
        e_s_expand = await bls.utils.expandMessageXMD(e_s_octs, expand_dst, 2 * TEMP_EX_LEN);
    } else {
        e_s_expand = expandMessageXOF(e_s_octs, expand_dst, 2 * TEMP_EX_LEN);
    }
    // 7.  if e_s_expand is INVALID, return INVALID
    // 8.  e = hash_to_scalar(e_s_expand[0..(expand_len - 1)])
    let e = await hash_to_scalar(e_s_expand.slice(0, TEMP_EX_LEN), dst, hashType);
    // 9.  s = hash_to_scalar(e_s_expand[expand_len..(expand_len * 2 - 1)])
    let s = await hash_to_scalar(e_s_expand.slice(TEMP_EX_LEN, 2 * TEMP_EX_LEN), dst, hashType);
