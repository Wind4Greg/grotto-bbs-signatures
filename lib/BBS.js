...
const CIPHERSUITE_ID = 'BBS_BLS12381G1_XMD:SHA-256_SSWU_RO_H2G_HM2S_';
const CIPHERSUITE_ID_SHAKE = 'BBS_BLS12381G1_XOF:SHAKE-256_SSWU_RO_H2G_HM2S_';
...
export async function sign(SK, PK, header, messages, generators,
  ...
  const e = await hash_to_scalar(e_serial_octs, dst, hashType);
...
export async function proofGen(PK, signature, header, ph, messages,
  ...
  // **TODO** New proofGen stuff starts here.
  // **TODO**: Proof Initialization/ProofInit
  const domain = await calculate_domain(PK, L, generators.Q1, generators.H,
    header, hashType);
  ...
  const randScalars = await rand_scalars(3 + U);
  const [r1, r2, r3, ...mTildeU] = randScalars;
  // Abar = A * r1
  const Abar = A.multiply(r1);
  // Bbar = B * r1 - Abar * e
  const Bbar = B.multiply(r1).subtract(Abar.multiply(e));
  // C = Abar * r2 + Bbar * r3 + H_j1 * m~_j1 + ... + H_jU * m~_jU
  let C = Abar.multiply(r2).add(Bbar.multiply(r3));
  for(let j = 0; j < U; j++) {
    C = C.add(generators.H[undisclosed[j]].multiply(mTildeU[j]));
  }
  // c = calculate_challenge(Abar, Bbar, C, (i1, ..., iR),
  //     (msg_i1, ..., msg_iR), domain, ph)
  const disclosed_messages = messages.filter((msg, i) =>
    disclosed_indexes.includes(i)
  );
  const c = await calculate_challenge(Abar, Bbar, C, disclosed_indexes,
    disclosed_messages, domain, ph, hashType);
  // r4 = - r1^-1 (mod r)
  const r4 = bls.fields.Fr.neg(bls.fields.Fr.inv(r1));
  // r2^ = r2 + r4 * e* c (mod r)
  const r2Hat = bls.fields.Fr.add(r2, bls.fields.Fr.mul(e,
    bls.fields.Fr.mul(r4, c)));
  // r3^ = r3 + r4 * c (mod r)
  const r3Hat = bls.fields.Fr.add(r3, bls.fields.Fr.mul(r4, c));
  // for j in (j1, ..., jU): m^_j = m~_j + msg_j * c (mod r)
  const mHatU = [];
  for(let j = 0; j < U; j++) {
    const mHatj = bls.fields.Fr.add(mTildeU[j], bls.fields.Fr.mul(
      messages[undisclosed[j]], c));
    mHatU.push(mHatj);
  }
  // proof = (Abar, Bbar, r2^, r3^, (m^_j1, ..., m^_jU), c)
  // return proof_to_octets(proof)
  return proof_to_octets(Abar, Bbar, r2Hat, r3Hat, mHatU, c);
}
...
export async function proofVerify(PK, proof, header, ph, disclosed_messages,
  ...
  // (Abar, Bbar, c, r2^, r3^, commitments) = proof_result
  ...
  const {Abar, Bbar, r2Hat, r3Hat, mHatU, c} = proof_result;
  const R = disclosed_indexes.length;
  ...
  let D = generators.P1.add(generators.Q1.multiply(domain));
  for(let i = 0; i < R; i++) {
    D = D.add(
  ...
  let C = Abar.multiply(r2Hat).add(Bbar.multiply(r3Hat));
  for(let j = 0; j < U; j++) {
    C = C.add(generators.H[undisclosed[j]].multiply(mHatU[j]));
  }
  C = C.add(D.multiply(c));
  const cv = await calculate_challenge(Abar, Bbar, C, disclosed_indexes,
    disclosed_messages, domain, ph, hashType);
  if(c !== cv) {
...
function octets_to_proof(octets) {
  // recover (Abar, Bbar, r2Hat, r3Hat, mHatU, c) from octets
  const min_length = 2 * POINT_LENGTH + 3 * SCALAR_LENGTH; //+ U * SCALAR_LENGTH
  ...
  return {Abar, Bbar, r2Hat, r3Hat, mHatU, c};
}

// proof_to_octets(Abar, Bbar, c, r2Hat, r3Hat, mHatU);
function proof_to_octets(Abar, Bbar, r2Hat, r3Hat, mHatU, c) {
...
async function calculate_domain(PK, L, Q_1, H_Points, header,
  hashType = 'SHA-256'
) {
  let ciphersuite_id = CIPHERSUITE_ID;
  if(hashType === 'SHAKE-256') {
...
async function calculate_challenge(Abar, Bbar, C, i_array, msg_array,
...
export async function prepareGenerators(L, hashType = 'SHA-256') {
  ...
  let candidate;
  if(hashType === 'SHA-256') {
    v = await expand_message_xmd(gen_seed_P1, seed_dst, EXPAND_LEN, sha256);
    v = await expand_message_xmd(concat(v, i2osp(1, 8)), seed_dst, EXPAND_LEN,
      sha256);
    candidate = await bls.G1.hashToCurve(v, {DST: gen_dst_string});
  } else {
    v = expandMessageXOF(gen_seed_P1, seed_dst, EXPAND_LEN);
    v = expandMessageXOF(concat(v, i2osp(1, 8)), seed_dst, EXPAND_LEN);
    candidate = await bls.G1.hashToCurve(v, {
      DST: gen_dst_string,
      expand: 'xof',
      hash: shake256,
