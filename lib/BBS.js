...
export async function sign(SK, PK, header, messages, generators, hashType = "SHA-256") {
    ...
    let e_s_expand;
    if (hashType === "SHA-256") {
        e_s_expand = await expand_message_xmd(e_s_octs, expand_dst, 2 * SCALAR_LENGTH, sha256);
    } else {
...
export async function hash_to_scalar(msg_octets, dst, hashType = "SHA-256") {
    ...
    while (hashed_scalar == 0n) {
        ...
        let uniform_bytes;
        if (hashType === "SHA-256") {
            uniform_bytes = await expand_message_xmd(msg_prime, dst, EXPAND_LEN, sha256);
        } else {
...
export async function prepareGenerators(L, hashType = "SHA-256") {
    ...
    for (let i = 0; i < count; i++) {
        if (hashType === "SHA-256") {
            v = await expand_message_xmd(concat(v, i2osp(n, 4)), seed_dst, SEED_LEN, sha256);
        } else {
        ...
        if (hashType === "SHA-256") {
            ...
            candidate = await bls.G1.hashToCurve(v, { DST: gen_dst_string, expand: "xof", hash: shake256 });
        }
        if (i === 0) {
    ...
    if (hashType === "SHA-256") {
        ...
        v = await expand_message_xmd(concat(v, i2osp(1, 4)), seed_dst, SEED_LEN, sha256);
        candidate = await bls.G1.hashToCurve(v, { DST: gen_dst_string });
    } else {
